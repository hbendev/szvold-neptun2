...
<form class="container mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="POST" action="{{ route('login') }}" enctype="multipart/form-data">
    ...
    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('email') border-red-500 @enderror" name="email" id="email" type="email" placeholder="sfsdl@elte.ik.hu">
    @error('email')
