...
<form class="w-full max-w-lg mx-auto" method="POST" action="{{ route('taskCreate', $id) }}">
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
        ...
        <input value="{{ old('from') }}" autocomplete="from" class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('from') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="from" name="from" type="datetime-local">
        @error('from')
            <p role="alert" class="text-red-500 text-xs italic">{{ $message }}</p>
        @enderror
