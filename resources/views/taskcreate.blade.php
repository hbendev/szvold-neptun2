...
@section('content')
<form class="w-full max-w-lg mx-auto" method="POST" action="{{ route('taskCreate', $id) }}">
    @csrf
    <div class="flex flex-wrap -mx-3 mb-6">
        ...
        <input value="{{ old('name') }}" autocomplete="name" required class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('name') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="name" name="name" type="text" placeholder="Első beadandó">
        @error('name')
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
        ...
        <input value="{{ old('from') }}" autocomplete="from" required class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('from') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="from" name="from" type="datetime-local">
        @error('from')
            <p role="alert" class="text-red-500 text-xs italic">{{ $message }}</p>
        @enderror
        ...
        <div class="w-full md:w-1/2 px-3">
            <label required class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="to">
                Határidő
            </label>
            <input value="{{ old('to') }}" autocomplete="to" class="appearance-none block w-full bg-gray-200 text-gray-700  border border-gray-200 @error('to') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="to" type="datetime-local" name="to">
            @error('to')
                <p role="alert" class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
